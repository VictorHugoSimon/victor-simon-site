...
test('build de staging injeta API, URL canônica, automação Growth e bloqueia robôs', async () => {
  ...
  const [html, panel, automationUi, robots, headers] = await Promise.all([
...
test('build de produção mantém indexação e automação do painel', async () => {
